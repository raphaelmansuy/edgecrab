...
> **已验证来源：** `crates/edgecrab-cli/src/main.rs` ·
...
`edgecrab-cli` 是最重的 crate — 它导入了所有东西。这是有意的。CLI 是*组合根*：代码库中唯一将所有部分连接成可运行整体的地方。

当启动失败或 agent 接线不正确时，bug 几乎总是在 `main.rs` 或 `runtime.rs` 中，而不是在 TUI widgets 或工具实现中。

🦀 *CLI 是螃蟹的大脑干 — 它连接所有神经通路但不实现思考。*

**参考：** [clap docs](https://docs.rs/clap) ·
...
`edgecrab claw migrate` 现在镜像 Hermes 的 OpenClaw entrypoint 而不是别名 Hermes importer。它导入 EdgeCrab 原生 OpenClaw 数据并归档不支持的 OpenClaw 仅配置以供手动审查。

`--config <PATH>` 不仅仅是文件覆盖。该配置文件的父目录成为兄弟 `.env`、`state.db`、plugins、skills 和其他二进制命令状态的有效运行时主目录。

---

## Slash commands

Slash commands are handled in `commands.rs`, but the user-facing metadata now lives in the shared `edgecrab-command-catalog` crate so the TUI help and the gateway help no longer maintain separate hardcoded tables.
...
This keeps the command definitions DRY without creating an illegal dependency from the gateway back into the CLI crate.

For argv parity without an explosion of one-off clap subcommands, EdgeCrab now uses `edgecrab slash <command...>` as the generic bridge into the same `CommandRegistry` used by the TUI. That preserves one command grammar and one handler graph.
...
- Agent construction in `runtime.rs` → [Agent 结构](../003_agent_core/001_agent_struct.md)
- Config loading → [配置和状态](../009_config_state/001_config_state.md)
- Profile paths → [配置和状态](../009_config_state/001_config_state.md)
- 安全 approval flow → [安全](../011_security/001_security.md)
