...
exec(open(os.path.expanduser(
    "~/.edgecrab/skills/red-teaming/godmode/scripts/load_godmode.py"
)).read())
...
1. **Reads `~/.edgecrab/config.yaml`** to detect the current model
...
7. **If a strategy works**, locks it in:
   ...
   - Writes prefill messages to `~/.edgecrab/prefill.json`
...
Then set in `~/.edgecrab/config.yaml`:
```yaml
agent:
...
exec(open(os.path.expanduser("~/.edgecrab/skills/red-teaming/godmode/scripts/parseltongue.py")).read())
...
```python
# Via execute_code
exec(open(os.path.expanduser("~/.edgecrab/skills/red-teaming/godmode/scripts/godmode_race.py")).read())
...
12. **execute_code sandbox has no env vars** — When Hermes runs auto_jailbreak via execute_code, the sandbox doesn't inherit `~/.edgecrab/.env`. Load dotenv explicitly: `from dotenv import load_dotenv; load_dotenv(os.path.expanduser("~/.edgecrab/.env"))`
