...
```bash
GSETUP="python ~/.edgecrab/skills/productivity/google-workspace/scripts/setup.py"
```
...
- Token is stored at `~/.edgecrab/google_token.json` and auto-refreshes.
- Pending OAuth session state/verifier are stored temporarily at `~/.edgecrab/google_oauth_pending.json` until exchange completes.
...
```bash
GAPI="python ~/.edgecrab/skills/productivity/google-workspace/scripts/google_api.py"
```
