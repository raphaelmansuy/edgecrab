...
- Config: `~/.edgecrab/config.yaml`
- API keys: `~/.edgecrab/.env`
- Skills: `~/.edgecrab/skills/`
- Hermes install: `~/.edgecrab/hermes-agent/`
- Venv: `~/.edgecrab/hermes-agent/venv/`
...
edgecrab skills install NAME      Install a skill from the hub
edgecrab skills config            Enable/disable skills per platform
```
...
```bash
edgecrab skills list              # Show installed skills
edgecrab skills search "docker"   # Search the hub
edgecrab skills install NAME      # Install from hub
edgecrab skills config            # Enable/disable per platform
```

### Per-platform skill control:

`edgecrab skills config` opens an interactive UI where you can enable or disable specific skills for each platform (cli, telegram, discord, etc.). Disabled skills won't appear in the agent's available skills list for that platform.
...
```bash
cd ~/.edgecrab/hermes-agent && source venv/bin/activate
...
The main config file is `~/.edgecrab/config.yaml`. Key sections:
...
1. Check `edgecrab skills list` shows the skill
2. Check `edgecrab skills config` has it enabled for your platform
