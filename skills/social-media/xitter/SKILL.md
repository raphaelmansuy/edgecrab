...
If you already keep your X credentials in `~/.edgecrab/.env`, the cleanest setup is:

```bash
mkdir -p ~/.config/x-cli
ln -sf ~/.edgecrab/.env ~/.config/x-cli/.env
```
...
- **Credential drift**: if you rotate tokens in `~/.edgecrab/.env`, make sure `~/.config/x-cli/.env` still points at the current file.
