...
```bash
SCRIPT="$(find ~/.edgecrab/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
edgecrab skills install official/productivity/telephony
```
...
3. Buy it and save it into `~/.edgecrab/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
3. persist that number to `~/.edgecrab/.env`
