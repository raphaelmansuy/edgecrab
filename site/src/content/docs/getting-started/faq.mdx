...
On launch and postinstall it also renames strictly older native `edgecrab` entries on your `PATH` (for example a leftover `~/.cargo/bin/edgecrab`) to `edgecrab.<ver>.bak` so they cannot shadow the upgraded install.
Set `EDGECRAB_NO_PATH_REPAIR=1` to disable that rename.

If you still see an older version, run `which -a edgecrab` and remove or reorder the unexpected install.
